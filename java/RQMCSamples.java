...
public class RQMCSamples extends RQMCExperiment {
    ...
    static int a14[] = {1, 6229, 2691, 3349, 5893, 7643, 7921, 7055, 4829, 5177, 5459, 4863, 
                 4901, 2833, 2385, 3729, 981, 957, 4047, 1013, 1635, 2327, 7879, 2805, 
                 2353, 1081, 3999, 879, 5337, 7725, 4889, 5103};
    
    /**
     * Writes a summary report with mean, variance, etc., to a .sum 
     * file with the given name, in given directory.
     */
    public static void reportToFile (TallyStore tally, String fileName) 
    		throws IOException {
     ...
     * with a line feed after each observation.
     */
    ...
    /**
     * Writes the sorted observations in `tally` as a `String` and 
     * prints them in a file with the given name, in given directory.
     * Also calls `reportToFile`. 
     */
    public static void dataToFile (TallyStore tally, String fileName) 
    		throws IOException {
    ...
    /**
	 * Performs m independent RQMC replications and save the sorted 
	 * output in the `statReps` collector. 
	 */
	public static void simulRepsRQMCSort (MonteCarloModelDouble model, PointSet p, 
			PointSetRandomization rand, int m, TallyStore statReps) {
		statReps.init();
	...
	public static void simulRepsAllTypes (MonteCarloModelDoubleTag model, 
			int s, int k, int m) throws IOException {
		...
		randnet = new NestedUniformScrambling(stream);
	    ...
	    System.out.println ("Total time for simulRepsAllTypes: " 
	        + timer.format() + "\n=========================================== \n");
	}
	...
	// The sorted values are saved in a file for each of the 5 types.
	public static void simulRepsNUS (MonteCarloModelDoubleTag model, 
			int s, int k, int m) throws IOException {
		TallyStore statReps;
		RandomStream stream = new LFSR113();
		String modelTag = model.getTag();
	...
	// The sorted values are saved in a file for each of the 5 types.
	public static void simulRepsLMS (MonteCarloModelDoubleTag model, 
			int s, int k, int m) throws IOException {
		TallyStore statReps;
		RandomStream stream = new LFSR113();
		String modelTag = model.getTag();
